...
build_bindings([{VarName, Value}|T], Accum) ->
    ...
    build_bindings(T, [[FinalVarName, "=", couch_util:json_encode(Value), ";"]|Accum]).

build_arg_list([], Accum) ->
    lists:reverse(Accum);
build_arg_list([H|[]], Accum) ->
    build_arg_list([], [couch_util:json_encode(H)|Accum]);
build_arg_list([H|T], Accum) ->
    build_arg_list(T, [[couch_util:json_encode(H), ","]|Accum]).
